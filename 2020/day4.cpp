...
bool passportIsValid(const bool testResults[7]) {
    for (int i = 0; i < 7; i++) {
        if (!testResults[i]) {
            return false;
        }
...
int checkPassports(const vector<string> &passportDataLines) {
    static vector<string> mandatoryFields = { "byr:", "iyr:", "eyr:", "hgt:", "hcl:", "ecl:", "pid:" };
    int count = 0;
    bool fieldFound[7] = { false };
    for (const auto &passportDataLine: passportDataLines) {
        if (passportDataLine.empty()) {
            // It's the beginning of a new passport.
            if (passportIsValid(fieldFound)) {
                count++;
            }
            for (int i = 0; i < 7; i++) {
                fieldFound[i] = false;
        ...
        // Try to find each of the passport fields.
        for (int i = 0; i < mandatoryFields.size(); i++) {
            if (passportDataLine.find(mandatoryFields[i]) != string::npos) {
                fieldFound[i] = true;
            }
        }
    }
...
int checkPassportsMoreStrictly(const vector<string> &passportDataLines) {
    static vector<string> mandatoryFields = { "byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid" };
    int count = 0;
    bool fieldIsValid[7] = { false };
    bool skipThePassport = false;
    for (string passportDataLine: passportDataLines) {
        if (skipThePassport || passportDataLine.empty()) {
            // It's the beginning of a new passport.
            if (passportIsValid(fieldIsValid)) {
                count++;
            }
            for (int i = 0; i < 7; i++) {
                fieldIsValid[i] = false;
            }
            skipThePassport = false;
            continue;
        }
        replace(passportDataLine.begin(), passportDataLine.end(), ':', ' ');
        vector<string> passportBits = split(passportDataLine, ' ');
        for (int i = 0; i < passportBits.size(); i += 2) {
            if (passportBits[i] == "byr") {
                int byrValue = stoi(passportBits[i + 1]);
                if (byrValue >= 1920 && byrValue <= 2002) {
                    fieldIsValid[0] = true;
                } else {
                    skipThePassport = true;
                }

            } else if (passportBits[i] == "iyr") {
                int iyrValue = stoi(passportBits[i + 1]);
            ...
            } else if (passportBits[i] == "eyr") {
                int eyrValue = stoi(passportBits[i + 1]);
            ...
            } else if (passportBits[i] == "hgt") {
                string hgtValue = passportBits[i + 1];
                int hgtNumber = stoi(hgtValue);
                if (hgtValue.find("cm") != string::npos) {
                    if (hgtNumber >= 150 && hgtNumber <= 193) {
                        fieldIsValid[3] = true;
                    } else {
                        skipThePassport = true;
                    }
                } else if (hgtValue.find("in") != string::npos) {
            ...
            } else if (passportBits[i] == "hcl") {
                static regex hclRegex("#[0-9a-f]{6}");
                smatch matches;
                if (regex_search(passportBits[i + 1], matches, hclRegex)) {
                    fieldIsValid[4] = true;
                } else {
                    skipThePassport = true;
                }

            } else if (passportBits[i] == "ecl") {
                static vector<string> eclValidValues = { "amb", "blu", "brn", "gry", "grn", "hzl", "oth" };
                if (find(eclValidValues.begin(), eclValidValues.end(), passportBits[i + 1]) != eclValidValues.end()) {
                    fieldIsValid[5] = true;
                } else {
                    skipThePassport = true;
                }

            } else if (passportBits[i] == "pid") {
                static regex pidRegex("[0-9]{9}");
                smatch matches;
                if (regex_search(passportBits[i + 1], matches, pidRegex) && passportBits[i + 1].length() == 9) {
                    fieldIsValid[6] = true;
                } else {
                    skipThePassport = true;
                }
...
void runDay4() {
    ...
    const vector<string> passportsData = {
    ...
    cout << "Day 4: checkPassports = " << checkPassports(passportsData);
    cout << ", checkPassportsMoreStrictly = " << checkPassportsMoreStrictly(passportsData) << endl;
}